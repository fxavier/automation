...
class ExcelToCSVConverter(QMainWindow):
    ...
    def convert_excel_to_csv(self):
        ...
        try:
            ...
            self.progress_bar.setValue(len(sheet_names))
    ...
    def merge_with_orgunits(csv_filename):
        try:
            ...
            QMessageBox.critical('Error', f'Key error: {e}')
        except Exception as e:
            QMessageBox.critical('Error', f'An error occurred while merging with orgunits: {e}')

    def unpivot_columns():
        initial_files_dir = os.path.join(os.path.dirname(__file__), 'initial_files')
    ...
    def concat_final_files(transformed_files):
        df_list = []
        for transformed_file in transformed_files:
            try:
                df = pd.read_csv(os.path.join('Transformed', transformed_file))
                df_list.append(df)
            except Exception as e:
                QMessageBox.critical('Error', f'An error occurred while concatenating files: {e}')
        if not df_list:
            raise ValueError("No valid files to concatenate.")
        final_df = pd.concat(df_list).drop_duplicates().reset_index(drop=True)
        final_df.to_csv('Final_Files/concatenated_file.csv', index=False)
        return final_df
    ...
    df.to_csv('Merged/file_import.csv', index=False)
